...
.btn-detail:hover {
  ...
  <body>
    ...
    <input type="hidden" name="resume_rno" value="${vo.resume_rno}">	
    <input type="hidden" name="posting_no" value="${vo.posting_pno}">	
      ...
      <div class="right">
        <div class="tabs">
          <button class="tab" onclick="openTab0">지원자 현황</button>
          <button class="tab" onclick="openTab1">추천서비스</button>

        </div>
        <div class="tab-content">
          <div class="tab-panel">
          	<h4>내가 작성한 공고에 지원한 지원자</h4>
          	<ul class="job-list">
		        <li>
          <table>
            <thead>
...
<%@include file="/WEB-INF/include/Footer.jsp"%>
 ...
 <script>
 document.addEventListener('DOMContentLoaded', function() {
	    var toggleBookmarkButtons = document.getElementsByName('bookmark');
	    	    
	    toggleBookmarkButtons.forEach(function(toggleBookmarkButton) {
	        toggleBookmarkButton.addEventListener('click', function(e) {
	
	        	bookmark = e.target;

	        	let resume_rno  = bookmark.getAttribute("data-resume_rno");
	        	let posting_pno = bookmark.getAttribute("data-posting_pno");
	        	let com_id = bookmark.getAttribute("data-com_id");
	            // fetch 요청
	            fetch('/Company/toggleBookmark', {
	            	  method: "POST",
	            	  headers: {
	            	    "Content-Type": "application/json",
	            	  },
